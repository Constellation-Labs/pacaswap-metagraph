...
import org.amm_metagraph.shared_data.services.combiners.OneTimeFixesHandler
...
import org.amm_metagraph.shared_data.types.States.{AmmCalculatedState, AmmOnChainState, RewardsState}
...
object IncidentTokenLockRemediationSpec extends MutableIOSuite {
  ...
  test("the production combiner fires only at the activation ordinal and strips the incident locks there") { _ =>
    val rewards = RewardsState().copy(
      availableRewards = RewardInfo(
        SortedMap(AddressAndRewardType(owner, RewardType.Governance) -> Amount(NonNegLong.unsafeFrom(123L)))
      )
    ...
    val state = DataState(
      ...
      AmmCalculatedState(votingPowers = votingPowers, rewards = rewards)
    )
    val beforeActivation = ordinal(ProtocolActivation.incidentTokenLockRemediation.value.value - 1L)

    for {
      ordinalR <- SignallingRef.of[IO, SnapshotOrdinal](SnapshotOrdinal.MinValue)
      handler = OneTimeFixesHandler.make[IO](ordinalR)
      fixed <- handler.handleOneTimeFixesOrdinals(state, ProtocolActivation.incidentTokenLockRemediation)
      untouched <- handler.handleOneTimeFixesOrdinals(state, beforeActivation)
    } yield expect.all(
      fixed.isDefined,
      fixed.exists(s => s.calculated.votingPowers(owner).info == SortedSet(legitimateInfo)),
      fixed.exists(s => !s.calculated.votingPowers.contains(incidentOnlyOwner)),
      fixed.exists(s => s.calculated.rewards == rewards),
      untouched.isEmpty
    )
  }
}
