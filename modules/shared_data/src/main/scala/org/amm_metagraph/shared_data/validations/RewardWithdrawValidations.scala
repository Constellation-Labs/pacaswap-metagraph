...
object RewardWithdrawValidations {
  def rewardWithdrawValidationL1[F[_]: Async](rewardWithdrawUpdate: RewardWithdrawUpdate): F[DataApplicationValidationErrorOr[Unit]] =
    valid.pure[F]

  def rewardWithdrawValidationL0[F[_]: Async: SecurityProvider](
