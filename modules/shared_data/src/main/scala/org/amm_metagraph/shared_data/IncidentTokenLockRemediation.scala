...
object IncidentTokenLockRemediation {
  ...
  /** Filters the incident locks after every active-lock import. A holder remains in the map when it has any unrelated lock, and its total
    * is recomputed from the retained `VotingPowerInfo` entries. Below activation this is the identity to preserve historical replay.
    */
