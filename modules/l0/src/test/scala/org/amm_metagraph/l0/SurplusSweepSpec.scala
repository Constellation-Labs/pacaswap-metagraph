...
/** The surplus sweep moves tokens out of an address nobody holds a key for.
  *
  * That makes it irreversible in both directions: we cannot recall it, and we cannot recover it
  * from a wrong destination. Every one of these assertions exists so that a mistake stops the
  * node instead of sending tokens somewhere they can never be retrieved from.
  */
object SurplusSweepSpec extends SimpleIOSuite {
  ...
  test("the shipped resource REFUSES to load while the destination is a placeholder") {
    // This is the assertion that matters most today. Until an operator sets a real destination,
    // the node must not build the snapshot at all.
    IO(loadSweep("up-surplus-sweep.json")).map { r =>
      expect.all(
        r.isFailure,
        r.failed.get.getMessage.contains("placeholder")
      )
    }
  }

  test("the shipped resource carries the measured surplus, source and currency") {
    // Read the raw fields directly: the loader refuses to build the action, but the numbers in
    // the file still have to be the right ones when the destination is filled in.
    ...
    IO.fromEither(decode[RawSurplusSweep](json)).map { raw =>
      expect.all(
        ...
        raw.destination == DestinationPlaceholder
      )
    }
  }
  ...
  test("the sweep is emitted at 731649, and at no neighbouring ordinal") {
    ...
    IO {
      ...
      expect.all(
        ...
        at(731649L).isFailure,                    // the sweep, currently blocked by the placeholder
