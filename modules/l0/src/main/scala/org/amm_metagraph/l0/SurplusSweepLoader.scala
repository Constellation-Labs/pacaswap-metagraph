...
/** Moves a token surplus out of the metagraph's custody address.
  *
  * The custody address has no private key. It is derived from the metagraph id, nobody holds a key for it, and it has never signed a
  * transaction on any ledger. Value leaves it only through a SpendAction the metagraph itself emits, which in normal operation happens in
  * exactly one place: paying out an LP withdrawal. So a surplus sitting there cannot be transferred back by hand, and cannot be burned by
  * hand either. This is the mechanism for it.
  *
  * Used for The Upsider AI: the wallet holds 71,706,224.58112005 UP that no pool's book claims. Sweeping it leaves the pool's book
  * untouched, so the pool reaches reserve == wallet with no price movement, and the treasury keeps the tokens. The alternative was booking
  * the surplus into the pool, which would have halved that pool's UP price.
  *
  * FAIL CLOSED. Every field is validated and a placeholder destination is rejected. Sending tokens to a wrong address is irreversible: the
  * destination cannot be recovered any more than the source can. If this resource is missing, malformed, or still carries the placeholder,
  * the node must refuse to build the snapshot rather than send anything anywhere.
  *
  * It is also fail-safe in the other direction: if the SpendAction never settles, the pool is merely over-backed, which is harmless. It can
